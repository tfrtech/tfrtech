(function(){const e=document.createElement("link").relList;if(e&&e.supports&&e.supports("modulepreload"))return;for(const a of document.querySelectorAll('link[rel="modulepreload"]'))c(a);new MutationObserver(a=>{for(const s of a)if(s.type==="childList")for(const o of s.addedNodes)o.tagName==="LINK"&&o.rel==="modulepreload"&&c(o)}).observe(document,{childList:!0,subtree:!0});function i(a){const s={};return a.integrity&&(s.integrity=a.integrity),a.referrerPolicy&&(s.referrerPolicy=a.referrerPolicy),a.crossOrigin==="use-credentials"?s.credentials="include":a.crossOrigin==="anonymous"?s.credentials="omit":s.credentials="same-origin",s}function c(a){if(a.ep)return;a.ep=!0;const s=i(a);fetch(a.href,s)}})();const b=document.querySelector("#app"),t={loading:!0,error:"",step:"intro",table:null,store:{storeName:"Cardápio Digital",logoEmoji:"🥟",logoUrl:null},products:[],categories:[],cart:[],customerName:"",customerPhone:"",sending:!1,statusMessage:"Aguardando preenchimento do cliente.",supabase:null};function v(r){return r.replace(/\D/g,"").slice(0,11)}function N(r){const e=v(r);return e.length<=2?e:e.length<=6?`(${e.slice(0,2)}) ${e.slice(2)}`:e.length<=10?`(${e.slice(0,2)}) ${e.slice(2,6)}-${e.slice(6)}`:`(${e.slice(0,2)}) ${e.slice(2,7)}-${e.slice(7)}`}function f(r){return new Intl.NumberFormat("pt-BR",{style:"currency",currency:"BRL"}).format(r)}function n(r){return String(r).replaceAll("&","&amp;").replaceAll("<","&lt;").replaceAll(">","&gt;").replaceAll('"',"&quot;").replaceAll("'","&#39;")}function E(){const r=window.location.pathname.split("/").filter(Boolean),e=r.indexOf("mesa");if(e===-1||r.length<e+3)return null;const i=Number(r[e+1]),c=r[e+2];return!Number.isInteger(i)||!c?null:{tableId:i,tableCode:c}}function $(r,e=[]){return`${r}:${e.map(i=>i.id).sort().join(",")}`}function y(){return t.cart.reduce((r,e)=>r+e.quantity,0)}function _(){return t.cart.reduce((r,e)=>{const i=e.complements.reduce((c,a)=>c+Number(a.price),0);return r+(Number(e.product.price)+i)*e.quantity},0)}function A(){throw new Error("Defina VITE_SUPABASE_URL e VITE_SUPABASE_PUBLISHABLE_KEY na Vercel.")}function l(){var c,a;if(!b)return;if(t.loading){b.innerHTML=`
            ...
            ${o.map(u=>{const p=t.cart.find(d=>d.product.id===u.id&&d.complements.length===0),m=p?p.quantity:0,g=u.stock>0;return`
                  <article class="product-card">
                    <div class="product-image">
                      ${u.image?`<img src="${n(u.image)}" alt="${n(u.name)}">`:`<span>${n(u.name.slice(0,1).toUpperCase())}</span>`}
                    </div>
                    <div class="product-main">
                      <p class="product-name">${n(u.name)}</p>
                      ${u.description?`<p class="product-desc">${n(u.description)}</p>`:""}
  ...
  `,h()}function h(){const r=document.querySelector("#customer-name"),e=document.querySelector("#customer-phone"),i=document.querySelector("#customer-form");r&&r.addEventListener("input",c=>{t.customerName=c.target.value;const a=document.querySelector('#customer-form button[type="submit"]');a&&(a.disabled=!t.customerName.trim()||v(t.customerPhone).length<10)}),e&&e.addEventListener("input",c=>{t.customerPhone=N(c.target.value),c.target.value=t.customerPhone;const a=document.querySelector('#customer-form button[type="submit"]');a&&(a.disabled=!t.customerName.trim()||v(t.customerPhone).length<10)}),i&&i.addEventListener("submit",c=>{if(c.preventDefault(),!t.customerName.trim()||v(t.customerPhone).length<10){t.statusMessage="Preencha nome e celular com um número válido antes de continuar.",l();return}t.step="menu",t.statusMessage="Cliente identificado. Agora escolha seus produtos.",l()}),document.querySelectorAll("[data-action]").forEach(c=>{c.addEventListener("click",M)})}async function M(r){const e=r.currentTarget.getAttribute("data-action"),i=r.currentTarget.getAttribute("data-product-id"),c=r.currentTarget.getAttribute("data-key");if(e==="new-order"||e==="back-menu"){t.step="menu",l();return}if(e==="open-cart"){t.step="cart",l();return}if(e==="send-order"){await L();return}if(e==="add-product"||e==="increase-product"){const a=t.products.find(o=>o.id===i);if(!a)return;const s=t.cart.find(o=>o.product.id===i&&o.complements.length===0);if(s){if(s.quantity>=a.stock){t.statusMessage="Estoque insuficiente para adicionar mais unidades.",l();return}s.quantity+=1}else t.cart.push({product:a,quantity:1,complements:[]});t.statusMessage="Produto adicionado ao carrinho.",l();return}if(e==="decrease-product"){const a=t.cart.find(s=>s.product.id===i&&s.complements.length===0);if(!a)return;a.quantity-=1,t.cart=t.cart.filter(s=>s.quantity>0),l();return}if(e==="increase-cart"||e==="decrease-cart"){const a=t.cart.find(s=>$(s.product.id,s.complements)===c);if(!a)return;if(e==="increase-cart"&&a.quantity>=Number(a.product.stock)){t.statusMessage="Estoque insuficiente para adicionar mais unidades.",l();return}a.quantity+=e==="increase-cart"?1:-1,t.cart=t.cart.filter(s=>s.quantity>0),l()}}async function L(){if(!t.supabase||!t.table||t.cart.length===0)return;t.sending=!0,t.statusMessage="Enviando pedido...",l();const r={table_id:t.table.tableId,table_name:t.table.name,customer_name:t.customerName.trim(),customer_phone:v(t.customerPhone),items:t.cart.map(i=>({product_id:i.product.id,product_name:i.product.name,product_price:i.product.price,quantity:i.quantity,complements:[]})),total:_(),status:"pending",source:"customer"},{error:e}=await t.supabase.from("customer_orders").insert(r);if(t.sending=!1,e){console.error(e),t.statusMessage="Erro ao enviar o pedido. Tente novamente.",l();return}t.step="sent",t.cart=[],t.statusMessage="Pedido enviado com sucesso.",l()}async function P(){var e,i;const r=E();if(!r){t.loading=!1,t.error="A URL da mesa está incompleta. Use o QR Code com /mesa/:id/:codigo.",l();return}try{t.supabase=A();const[c,a,s,o]=await Promise.all([t.supabase.rpc("get_public_table_by_qr",{p_table_id:r.tableId,p_table_code:r.tableCode}),t.supabase.rpc("get_public_menu_products"),t.supabase.rpc("get_public_menu_categories"),t.supabase.rpc("get_public_store_settings")]);if(c.error)throw c.error;if(a.error)throw a.error;if(s.error)throw s.error;if(o.error)throw o.error;const u=(e=c.data)==null?void 0:e[0];if(!u){t.loading=!1,t.error="Mesa não encontrada. Verifique se o QR Code foi gerado corretamente.",l();return}t.table=u,t.products=(a.data??[]).map(d=>({id:d.id,name:d.name,description:d.description??"",price:Number(d.price),category:d.category,stock:Number(d.stock),image:d.image??""}));const p=new Map((s.data??[]).map(d=>[d.name,Number(d.sort_order)])),m=new Set([...(s.data??[]).map(d=>d.name),...t.products.map(d=>d.category)]);t.categories=[...m].sort((d,q)=>(p.get(d)??999)-(p.get(q)??999));const g=(i=o.data)==null?void 0:i[0];g&&(t.store.storeName=g.store_name??t.store.storeName,t.store.logoEmoji=g.logo_emoji??t.store.logoEmoji,t.store.logoUrl=g.logo_url??null),t.loading=!1,l()}catch(c){console.error(c),t.loading=!1,t.error="Não foi possível carregar o cardápio agora. Tente novamente em instantes.",l()}}l();P();
